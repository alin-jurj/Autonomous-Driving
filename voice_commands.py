...
##### GO TO DESTINATION
    def goToDestination(self, robot: cozmo.robot.Robot = None, cmd_args=None):
        ...
        if (entire_message is not None) and (len(entire_message) > 0):
            if(latest_image):
                #text = pytesseract.image_to_string(PIL.Image.open("cozmo_pic_1668855737.png"))
                #print(text)
                robot.say_text("This").wait_for_completed()
            #robot.drive_straight(distance_mm(150), speed_mmps(50)).wait_for_completed()
            return 'I drove until"' + entire_message + '"!'
